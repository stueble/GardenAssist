...
function renderCalendar(garden = MOCK_GARDEN) {
    return render(_jsx(I18nextProvider, { i18n: i18n, children: _jsx(CalendarView, { garden: garden, loading: garden === null, invalidateGarden: vi.fn() }) }));
}
...
describe("CalendarView — plant rows (AC #1, #3)", () => {
    ...
    it("shows plant common name", async () => {
        renderCalendar();
        await waitFor(() => expect(screen.getByText("Rote Rose")).toBeInTheDocument());
    });
...
describe("CalendarView — search (AC #5)", () => {
    ...
    it("shows 'Keine Pflanzen' when search has no match", async () => {
        ...
        await waitFor(() => expect(screen.getByText("Keine Pflanzen gefunden.")).toBeInTheDocument());
    });
});
...
describe("CalendarView — overlapping bars rendered in lanes (AC #1, #6)", () => {
    ...
    const GARDEN_OVERLAP = {
        plan_url: null, plan_name: null,
        plants: [MOCK_PLANT_OVERLAP],
        attachments: [], journal_entries: [], warnings: [],
    };
    it("both overlapping bars are rendered as separate DOM elements", async () => {
        render(_jsx(I18nextProvider, { i18n: i18n, children: _jsx(CalendarView, { garden: GARDEN_OVERLAP, loading: false, invalidateGarden: vi.fn() }) }));
        await waitFor(() => {

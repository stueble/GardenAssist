...
function renderSection(plan) {
    return render(_jsx(I18nextProvider, { i18n: i18n, children: _jsx(GardenPlanSection, { plan: plan }) }));
}
describe("GardenPlanSection — no plan", () => {
    ...
    it("shows dropzone when pending action is 'remove'", () => {
        renderSection(makePlanState({
            previewUrl: "/static/garden/plan.png",
            previewName: "gartenplan.png",
            pending: { type: "remove" },
            dirty: true,
        }));
        expect(screen.getByTestId("garden-plan-dropzone")).toBeInTheDocument();
...
describe("GardenPlanSection — plan exists (saved)", () => {
    it("shows the preview row when a plan exists (AC #2)", () => {
        renderSection(makePlanState({
            previewUrl: "/static/garden/plan.png",
            previewName: "gartenplan.png",
        }));
...
describe("GardenPlanSection — file input", () => {
    it("calls selectFile when a file is chosen (AC #1, #4)", async () => {
        ...
        await waitFor(() => expect(plan.selectFile).toHaveBeenCalledWith(file));
    });
});

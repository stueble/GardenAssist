...
function renderView(garden = MOCK_GARDEN) {
    return render(_jsx(MemoryRouter, { children: _jsx(I18nextProvider, { i18n: i18n, children: _jsx(PlantsView, { garden: garden, loading: garden === null, invalidateGarden: vi.fn() }) }) }));
}
// ── Tests ─────────────────────────────────────────────────────────────────────
describe("PlantsView — table rendering (AC #1)", () => {
    ...
    it("shows plant count in subheader", async () => {
        renderView();
        await waitFor(() => expect(screen.getByTestId("plants-count")).toHaveTextContent("3"));
    });
});
...
describe("PlantsView — sorting (AC #3)", () => {
    ...
    it("clicking Name header switches to desc — Rose first", async () => {
        ...
        fireEvent.click(screen.getByText(/^Pflanze/));
        const rows = screen.getAllByTestId("plant-row");
        const names = rows.map((r) => within(r).getAllByRole("cell")[0].textContent);
...
describe("PlantsView — cold protection badge in card (TASK-075 AC #5)", () => {
    it("shows 🏠 badge on card when temperature_protected is true", async () => {
        ...
        render(_jsx(I18nextProvider, { i18n: i18n, children: _jsx(PlantsView, { garden: garden, loading: false, invalidateGarden: vi.fn() }) }));

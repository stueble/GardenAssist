...
function renderJournal(garden = MOCK_GARDEN) {
    return render(_jsx(I18nextProvider, { i18n: i18n, children: _jsx(JournalView, { garden: garden, loading: garden === null, invalidateGarden: vi.fn() }) }));
}
...
describe("JournalView — timeline entries (AC #1, #2)", () => {
    ...
    it("shows entry title", async () => {
        renderJournal();
        await waitFor(() => expect(screen.getByText("Rosen geschnitten")).toBeInTheDocument());
    });
...
describe("JournalView — expand entry (AC #2)", () => {
    it("clicking entry with notes expands it", async () => {
        ...
        await waitFor(() => expect(screen.getByText("Frühjahrsschnitt durchgeführt.")).toBeInTheDocument());
    });
});
...
describe("JournalView — FAB opens new entry panel (story-036 AC #1)", () => {
    ...
    it("clicking FAB opens the entry panel", async () => {
        ...
        await waitFor(() => expect(screen.getByTestId("entry-panel")).toBeInTheDocument());
...
describe("JournalView — entry panel fields (story-036 AC #2, #3, #4)", () => {
    ...
    it("shows plant picker with plants (AC #3)", async () => {
        ...
        const select = screen.getByTestId("panel-plant");
...
describe("JournalView — close panel (story-036 AC #1)", () => {
    it("✕ button closes the panel", async () => {
        ...
        await waitFor(() => expect(screen.queryByTestId("panel-close")).not.toBeInTheDocument());
    });
});
describe("JournalView — edit existing entry (story-036 AC #7)", () => {
    it("edit button opens panel in edit mode", async () => {
        ...
        await waitFor(() => expect(screen.getByTestId("panel-save")).toBeInTheDocument());
    });
...
function renderJournalWithSchedules() {
    return render(_jsx(I18nextProvider, { i18n: i18n, children: _jsx(JournalView, { garden: GARDEN_WITH_SCHEDULES, loading: false, invalidateGarden: vi.fn() }) }));
}
describe("JournalView — TASK-050: schedule picker (AC #1)", () => {
    ...
    it("schedule picker shows only care schedules (not bloom)", async () => {
        ...
        fireEvent.change(screen.getByTestId("panel-plant"), { target: { value: "p1" } });
        await waitFor(() => screen.getByTestId("panel-schedule"));
        const select = screen.getByTestId("panel-schedule");
    ...
    it("createJournalEntry is called with correct schedule_id when schedule selected", async () => {
        ...
        fireEvent.click(screen.getByTestId("panel-save"));
        await waitFor(() => expect(apiClient.createJournalEntry).toHaveBeenCalledOnce());
        expect(apiClient.createJournalEntry).toHaveBeenCalledWith(expect.objectContaining({ schedule_id: "s1", entry_type: "done" }));
    });
    it("schedule_id is null and entry_type is manual when no schedule selected (AC #2)", async () => {
        ...
        fireEvent.click(screen.getByTestId("panel-save"));
        await waitFor(() => expect(apiClient.createJournalEntry).toHaveBeenCalledOnce());
        expect(apiClient.createJournalEntry).toHaveBeenCalledWith(expect.objectContaining({ schedule_id: null, entry_type: "manual" }));
    });
});

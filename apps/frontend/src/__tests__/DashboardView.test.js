...
function renderDashboard(garden = MOCK_GARDEN) {
    return render(_jsx(I18nextProvider, { i18n: i18n, children: _jsx(DashboardView, { garden: garden, loading: garden === null, invalidateGarden: vi.fn() }) }));
}
...
describe("DashboardView — garden plan (AC #1)", () => {
    it("renders GardenPlanWidget", async () => {
        renderDashboard();
        await waitFor(() => expect(screen.getByTestId("garden-plan-widget")).toBeInTheDocument());
    });
    it("renders plan image when plan_url is set", async () => {
        renderDashboard();
        await waitFor(() => expect(screen.getByTestId("garden-plan-img")).toBeInTheDocument());
    });
});
...
describe("DashboardView — zoom buttons (AC #6)", () => {
    it("renders fit-height button", async () => {
        renderDashboard();
        await waitFor(() => expect(screen.getByTestId("zoom-btn-fit-h")).toBeInTheDocument());
    });
    it("renders fit-width button", async () => {
        renderDashboard();
        await waitFor(() => expect(screen.getByTestId("zoom-btn-fit-w")).toBeInTheDocument());
    });
});
describe("DashboardView — legend (AC #7)", () => {
    it("renders the legend", async () => {
        renderDashboard();
        await waitFor(() => expect(screen.getByTestId("plan-legend")).toBeInTheDocument());
    });
});
describe("DashboardView — pin click opens detail panel (AC #5)", () => {
    it("clicking a plant pin opens the detail panel (close button visible)", async () => {
        ...
        await waitFor(() => expect(screen.getByTestId("detail-close")).toBeInTheDocument());
    });
...
describe("DashboardView — MonthBand (story-032)", () => {
    ...
    it("tooltip appears on month cell hover (AC #4)", async () => {
        ...
        await waitFor(() => expect(screen.queryByTestId("month-tooltip")).toBeInTheDocument());
    });
    it("tooltip disappears on mouse leave (AC #4)", async () => {
        ...
        await waitFor(() => expect(screen.queryByTestId("month-tooltip")).not.toBeInTheDocument());
    });
});
...
function makePlantForFrost(overrides = {}) {
    return {

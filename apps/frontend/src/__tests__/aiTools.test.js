...
import { registerPlantEditHandler, getPlantEditHandler, } from "../hooks/usePlantEditContext";
...
function renderWithRef(plant = null) {
    let capturedRef = null;
    function Wrapper() {
        const ref = useRef(null);
        capturedRef = ref;
        return (_jsx(I18nextProvider, { i18n: i18n, children: _jsx(PlantEditDialog, { ref: ref, plant: plant, onClose: vi.fn(), onSaved: vi.fn(), positions: [], onPositionsChange: vi.fn(), initialPositions: [], pickMode: false, onPickModeChange: vi.fn() }) }));
    }
    const result = render(_jsx(Wrapper, {}));
...
describe("PlantEditDialog — AI field markers (AC #3, #4, #5)", () => {
    ...
    it("reverts a field on × click and removes its marker (AC #4)", async () => {
        ...
        const waterSelect = screen.getByTestId("field-water");
...
describe("dispatchToolCallForTest", () => {
    ...
    it("returns empty string on successful dispatch", () => {
        const editPlant = vi.fn();
        const unregister = registerPlantEditHandler({ editPlant });
        const msg = dispatchToolCallForTest({ tool: "editPlant", id: null, fields: { name_common: "Test" } }, "de");
        expect(msg).toBe("");
...
describe("applyAiFields — schedule add (AC #1, #2)", () => {
    it("PlantEditFields type accepts schedules array", () => {
        ...
        const fields = {
            schedules: [
                { action: "add", schedule_type: "bloom", start_week: 17, end_week: 36, color: "#c0392b", label: "Hauptblüte", notes: null },
                { action: "add", schedule_type: "pruning", start_week: 9, end_week: 10 },
            ],
        };
...
import { registerJournalEditHandler, getJournalEditHandler, } from "../hooks/useJournalEditContext";
...
function renderJournalView(garden = MOCK_GARDEN) {
    return render(_jsx(I18nextProvider, { i18n: i18n, children: _jsx(JournalView, { garden: garden, loading: false, invalidateGarden: () => { } }) }));
}
describe("TASK-054 — openJournalEdit tool (AC #1)", () => {
    ...
    it("openJournalEdit prefills date field with AI marker", async () => {
        ...
        await waitFor(() => {
            const dateInput = screen.getByTestId("panel-date");
    ...
    it("openJournalEdit prefills title with AI marker", async () => {
        ...
        await waitFor(() => {
            const titleInput = screen.getByTestId("panel-title");
    ...
    it("openJournalEdit prefills notes with AI marker", async () => {
        ...
        await waitFor(() => {
            const notesInput = screen.getByTestId("panel-notes");
...
describe("TASK-054 — × revert restores previous value (AC #3 revert)", () => {
    it("clicking × on title reverts to original value and removes marker", async () => {
        ...
        await waitFor(() => {
            ...
            const input = screen.getByTestId("panel-title");
    ...
    it("clicking × on notes reverts to original value", async () => {
        ...
        await waitFor(() => {
            const input = screen.getByTestId("panel-notes");
            expect(input.value).toBe("");
...
describe("TASK-054 — dispatchToolCall for journal tools (AC #1 + #2)", () => {
    it("openJournalEdit tool returns empty string when handler registered", () => {
        ...
        const result = dispatchToolCallForTest({ tool: "openJournalEdit", prefill: { entry_type: "observation" } }, "de");
        expect(result).toBe("");
    ...
    it("openJournalEdit tool returns error when handler not registered", () => {
        ...
        unregister();
        const result = dispatchToolCallForTest({ tool: "openJournalEdit", prefill: {} }, "de");

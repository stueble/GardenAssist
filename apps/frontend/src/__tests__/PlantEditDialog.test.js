...
vi.mock("../api/client", () => ({
    apiClient: {
        ...
        updatePlant: vi.fn().mockImplementation((_id, data) => Promise.resolve({ id: "p1", ...data, tasks: [], journal_entries: [], schedules: [], attachments: [], positions: [], created_at: "", updated_at: "" })),
...
function renderDialog(plant = null, onClose = vi.fn(), onSaved = vi.fn(), extraProps = {}) {
    return render(_jsx(I18nextProvider, { i18n: i18n, children: _jsx(PlantEditDialog, { plant: plant, onClose: onClose, onSaved: onSaved, positions: [], onPositionsChange: vi.fn(), initialPositions: [], pickMode: false, onPickModeChange: vi.fn(), ...extraProps }) }));
}
/**
 ...
 */
function renderDialogWithState(plant = null, initialPositions = [], extraProps = {}) {
    function Wrapper() {
        const [positions, setPositions] = useState(initialPositions);
        return (_jsx(I18nextProvider, { i18n: i18n, children: _jsx(PlantEditDialog, { plant: plant, onClose: vi.fn(), onSaved: vi.fn(), positions: positions, onPositionsChange: setPositions, initialPositions: initialPositions, pickMode: false, onPickModeChange: vi.fn(), ...extraProps }) }));
    }
    return render(_jsx(Wrapper, {}));
}
...
function openSection(sectionText) {
    ...
    if (!header)
        throw new Error(`Section header not found: ${sectionText}`);
...
describe("PlantEditDialog — icon picker (AC #3)", () => {
    ...
    it("auto-suggests icon when category changes (AC #3)", async () => {
        ...
        await waitFor(() => expect(screen.getByTestId("icon-preview").textContent).toBe("🌳"));
    });
});
...
describe("PlantEditDialog — save and cancel (AC #6)", () => {
    it("shows error when saving without name", async () => {
        ...
        await waitFor(() => expect(screen.getByText(/erforderlich/i)).toBeInTheDocument());
    });
...
describe("PlantEditDialog — schedules included in save payload (AC #2)", () => {
    it("createPlant is called with schedules array when entries exist", async () => {
        ...
        fireEvent.click(screen.getByTestId("edit-save"));
        await waitFor(() => expect(apiClient.createPlant).toHaveBeenCalledOnce());
        const payload = apiClient.createPlant.mock.calls[0][0];
...
describe("PlantEditDialog — pre-filled positions (story-028 AC #3)", () => {
    it("renders pre-filled position rows when positions prop has entries", () => {
        ...
        const xInput = screen.getByTestId("position-x");
...
describe("PlantEditDialog — Bilder section renders (story-029 AC #1)", () => {
    ...
    it("shows category dropdown options including PDF/invoice (AC #2)", () => {
        ...
        const fileInput = document.querySelector('[data-testid="attachment-file-input"]');
        expect(fileInput).toBeTruthy();
    });
});
describe("PlantEditDialog — Bilder: pre-filled from existing plant (story-029 AC #1)", () => {
    const plantWithAttachment = {
        ...{
            id: "p1", name_common: "Rote Rose", name_botanical: "Rosa",
            icon: "🌹", origin_type: "native", category: "Strauch",
...
describe("PlantEditDialog — Bilder: local file added (story-029 AC #1)", () => {
    it("adding a local file creates an attachment row", () => {
        ...
        function Wrapper() {
            ...
            return (_jsx(I18nextProvider, { i18n: i18n, children: _jsx(PlantEditDialog, { plant: null, onClose: vi.fn(), onSaved: vi.fn(), positions: [], onPositionsChange: vi.fn(), initialPositions: [], pickMode: false, onPickModeChange: vi.fn() }) }));
        }
        render(_jsx(Wrapper, {}));
        openSection("Bilder & Dokumente");
        const fileInput = document.querySelector('[data-testid="attachment-file-input"]');
...
describe("PlantEditDialog — Bilder: delete saved attachment (story-029 AC #4)", () => {
    ...
    it("removes attachment row after successful deletion", async () => {
        ...
        expect(screen.getAllByTestId("attachment-row")).toHaveLength(1);
        fireEvent.click(screen.getByTestId("attachment-delete"));
        await waitFor(() => expect(screen.queryAllByTestId("attachment-row")).toHaveLength(0));
    });
});
...
describe("PlantEditDialog — AI schedule ID normalisation (regression)", () => {
    const PLANT_WITH_SCHEDULE = {
        ...MOCK_PLANT,
        schedules: [MOCK_SCHEDULE],
    };
    it("marks a schedule for removal when model sends plain UUID", async () => {
        const ref = createRef();
        render(_jsx(I18nextProvider, { i18n: i18n, children: _jsx(PlantEditDialog, { ref: ref, plant: PLANT_WITH_SCHEDULE, onClose: vi.fn(), onSaved: vi.fn(), positions: [], onPositionsChange: vi.fn(), initialPositions: [], pickMode: false, onPickModeChange: vi.fn() }) }));
        act(() => {
...
describe("PlantEditDialog — save after AI suggestions (regression)", () => {
    it("calls onSaved after applyAiFields + Save click", async () => {
        ...
        const onSaved = vi.fn();
        const ref = createRef();
        render(_jsx(I18nextProvider, { i18n: i18n, children: _jsx(PlantEditDialog, { ref: ref, plant: MOCK_PLANT, onClose: vi.fn(), onSaved: onSaved, positions: [], onPositionsChange: vi.fn(), initialPositions: [], pickMode: false, onPickModeChange: vi.fn() }) }));
    ...
    it("calls onPendingChange with data after applyAiFields", async () => {
        ...
        render(_jsx(I18nextProvider, { i18n: i18n, children: _jsx(PlantEditDialog, { ref: ref, plant: MOCK_PLANT, onClose: vi.fn(), onSaved: vi.fn(), onPendingChange: onPendingChange, positions: [], onPositionsChange: vi.fn(), initialPositions: [], pickMode: false, onPickModeChange: vi.fn() }) }));
        act(() => {
        ...
        await waitFor(() => {
            ...
            expect(lastCall.scalarFields.name_botanical).toBe("Rosa canina");
        });
    });

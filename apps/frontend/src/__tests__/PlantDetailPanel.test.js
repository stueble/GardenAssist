...
function renderPanel(overrides) {
    ...
    render(_jsx(I18nextProvider, { i18n: i18n, children: _jsx(PlantDetailPanel, { plant: MOCK_PLANT, onClose: onClose, onEdit: onEdit, onDelete: onDelete, ...overrides }) }));
...
describe("PlantDetailPanel — care notes (AC #4)", () => {
    ...
    it("does not render care notes section when null", () => {
        render(_jsx(I18nextProvider, { i18n: i18n, children: _jsx(PlantDetailPanel, { plant: { ...MOCK_PLANT, care_notes: null }, onClose: vi.fn() }) }));
...
describe("PlantDetailPanel — expandable task notes (TASK-064)", () => {
    ...
    function renderWithNotes() {
        return render(_jsx(I18nextProvider, { i18n: i18n, children: _jsx(PlantDetailPanel, { plant: PLANT_WITH_NOTES, onClose: vi.fn(), onEdit: vi.fn(), onDelete: vi.fn() }) }));
    }
...
describe("PlantDetailPanel — delete failure (story-040 AC #7)", () => {
    it("shows inline error when deletePlant rejects", async () => {
        const { apiClient } = await import("../api/client");
        apiClient.deletePlant.mockRejectedValueOnce(new Error("Network error"));
        renderPanel();
        fireEvent.click(screen.getByTestId("detail-btn-delete"));
        fireEvent.click(screen.getByTestId("detail-delete-ok"));
        await waitFor(() => expect(screen.getByTestId("detail-delete-error")).toBeInTheDocument());
    });
    it("hides confirmation dialog on failure but keeps panel open", async () => {
        const { apiClient } = await import("../api/client");
        apiClient.deletePlant.mockRejectedValueOnce(new Error("fail"));
        renderPanel();
        fireEvent.click(screen.getByTestId("detail-btn-delete"));
        fireEvent.click(screen.getByTestId("detail-delete-ok"));
        await waitFor(() => expect(screen.queryByTestId("detail-delete-confirm")).not.toBeInTheDocument());
...
describe("PlantDetailPanel — cold protection badge (TASK-075 AC #4)", () => {
    it("shows protected badge when temperature_protected is true", () => {
        const plant = { ...MOCK_PLANT, temperature_protected: true };
        render(_jsx(I18nextProvider, { i18n: i18n, children: _jsx(PlantDetailPanel, { plant: plant, onClose: vi.fn(), onEdit: vi.fn(), onDelete: vi.fn() }) }));

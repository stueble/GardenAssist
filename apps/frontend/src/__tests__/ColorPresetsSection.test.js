...
function renderSection(presets = [], onChange = vi.fn()) {
    return {
        onChange,
        ...render(_jsx(I18nextProvider, { i18n: i18n, children: _jsx(ColorPresetsSection, { form: makeSettings(presets), onChange: onChange }) })),
    };
}
...
describe("ColorPresetsSection — input editability", () => {
    it("name input is not blocked by draggable container: mousedown does not have stopPropagation that prevents focus", () => {
        ...
        const entryRow = input.closest("[data-testid='preset-entry']");
...
describe("ColorPresetsSection — interactions", () => {
    it("adds a new preset when 'Add color' is clicked (AC #3)", () => {
        ...
        const bloomPresets = patch.color_presets.filter((p) => p.schedule_type === "bloom");
    ...
    it("updates preset name on input change (AC #2)", () => {
        ...
        expect(onChange).toHaveBeenCalledOnce();
        const [patch] = onChange.mock.calls[0];
        const bloom = patch.color_presets.filter((p) => p.schedule_type === "bloom");
    ...
    it("onChange passes full color_presets array including other groups (AC #5)", () => {
        ...
        const [patch] = onChange.mock.calls[0];
        const allPresets = patch.color_presets;
